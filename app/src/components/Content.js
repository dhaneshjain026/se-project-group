...
const defaultTokenType = 'Type of token'
const defaultNetwork = 'Select Network'
const defaultPayment = 'Payment Currency'
...
export default class Content extends Component {
    ...
    async setPrice() {
        if (this.state.tokenType.includes('Basic')
            || this.state.tokenType.includes('Guy')
            || this.state.payment.includes('Currency')) {
            this.setState({
                price: '0.00',
                payment: defaultPayment
            })
        ...
        this.setState({
            ...
            paymentsList: this.state.paymentsList.map(
                payment => payment.key === 'bnb'
                    ? { key: 'bnb', name: `pay with ${this.state.networkSymbol}` }
                    : payment
            )
        })
    }
    ...
    getApproveOrCreateButton() {
        ...
        return <Button disabled={this.state.loading} css={{ marginLeft: '25%' }} bordered size="xl" color="primary" onClick={() => this.create()}>
            {this.state.loading ? <Loading type="points" color="currentColor" size="xl" /> : <>CREATE TOKEN</>}
        </Button>
    }
    ...
    openExplorer() {
        window.open(
            `https://sepolia.etherscan.io/${this.state.deployedAddress}`,
            '_blank',
            'noopener,noreferrer'
        )
    }
        ...
        return <Container css={{ mt: 50 }} gap={1}>
            <Row css={{
                ...
                <Col>
                    <h3>
                        Details of token<br />
                        <small className='details_text'>Enter the details of the token to create</small>
                    </h3>
                </Col>
                <Col></Col>
                <Col></Col>
                        ...
                        <Row>
                            ...
                            <Col>
                                <Button bordered size="sm" color="primary" onClick={() => this.state.displayAccount ? this.disconnect() : this.connect()}>
                                    {this.state.displayAccount ? <>{this.state.displayAccount} &nbsp;<Logout size={18} color="#0271f5" /></> : <>Connect</>}
                                </Button>
                            </Col>
                        </Row>
                    </Text>
                    ...
                    <Row gap={1} css={{ mt: 50 }}>
                        <Col>
                            <Input
                                ...
                                helperText="Choose a name for your token."
                                width="80%"
                                clearable
                                bordered
                            ...
                            <Input
                                ...
                                helperText="Choose a symbol for your token (typically 3 to 5 characters)."
                                width="80%"
                                clearable
                                bordered
                            ...
                            <Input
                                ...
                                helperText="Insert the number of decimal places in your token, if you don't know what to put, insert 18."
                                width="80%"
                                clearable
                                bordered
                                type="number"
                                ...
                                label="Number of decimals of the Token *"
                                placeholder="18" />
                            <Spacer y={1.5} />
                            <Input
                                onChange={e => this.setState({
                                ...
                                helperText="Insert the initial supply of your token, which will be sent to your connected account."
                                width="80%"
                                bordered
                                type="number"
                        ...
                        <Col css={{ marginTop: '10%' }}>
                            ...
                            <Spacer y={1.2} />
                            {
                                this.isFormValidated()
                                    ...
                                    : <Button disabled css={{ marginLeft: '25%' }} bordered size="xl" color="primary">CREATE TOKEN</Button>
                            }
                        </Col>
                    </Row> :
                    <Login />
                ...
                <Modal.Footer>
                    {
                        this.state.modalDeployed ?
                            <Button auto flat onClick={() => this.openExplorer()}>
                                View Token &nbsp;<OpenLink size={18} color="#0271f5" />
                            </Button> :
